...
export { ProArrayField } from "./array-field";
...
export type ProSchemaFormValues = Record<string, ProSchemaFormValue>;
...
export interface ProSchemaFormItem {
	...
	required?: boolean;
	disabled?: boolean;
	hidden?: boolean | ((values: ProSchemaFormValues) => boolean);
	tooltip?: ReactNode;
	description?: ReactNode;
	extra?: ReactNode;
	errors?: string[];
				...
				requestAnimationFrame(() => {
					content
						.querySelector<HTMLElement>(
							'form input:not([type="hidden"]):not(:disabled), form textarea:not(:disabled), form [role="combobox"]:not([aria-disabled="true"]), form [data-slot="switch"]:not(:disabled), form [data-slot="checkbox"]:not(:disabled)',
						)
						?.focus();
				});
			}}
		...
		<div className={cn("space-y-1.5", className)}>
			{label != null && (
				<div className="flex items-center gap-1.5">
					...
					{required && (
						<span
							className="text-sm leading-5 text-destructive-foreground"
							aria-hidden="true"
						>
							*
						</span>
...
function ProSchemaFields({
	...
	return (
		<div className={cn("space-y-4", className)}>
			{schema.map((item) => {
				const hidden =
					typeof item.hidden === "function" ? item.hidden(values) : item.hidden;
