...
GMShop Edge 是商城，不是支付网关。它不提供商户 API 凭证或网关订单协议，不扫描公链，
也不运行交易所/钱包收款适配器。GMpay 与 EPay 仅作为本商城订单的外部托管收银台适配器。
上述能力全部属于开源项目，不设置闭源 Pro 或 Enterprise 功能层。
...
如需使用上游串货，请参阅[供应商串货配置与故障恢复](docs/suppliers.zh-CN.md)。
